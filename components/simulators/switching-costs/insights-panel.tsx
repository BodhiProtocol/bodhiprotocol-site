...
interface InsightsPanelProps {
  dataToolsDelta: number;
  peopleHabitDelta: number;
...
function InsightsPanel({ dataToolsDelta, peopleHabitDelta, contractDelta, alternativeDelta }: InsightsPanelProps) {
  return (
    <GlassCard className="gap-5 p-6 sm:p-6">
      <Eyebrow className="text-brand">Insights</Eyebrow>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <DeltaStat label="Data & Tools" delta={dataToolsDelta} />
        <DeltaStat label="People & Habit" delta={peopleHabitDelta} />
        <DeltaStat label="Contract" delta={contractDelta} />
        <DeltaStat label="Alternative Pull" delta={alternativeDelta} />
      </div>
    </GlassCard>
  );
}
