...
const RELATED_MENTAL_MODELS = [
  ...
  { name: "Platform Businesses", description: "Why some companies win by connecting others, not by selling directly." },
];
  ...
  return (
    <GlassCard className="items-center gap-8 p-8 text-center sm:p-12">
      <div className="flex flex-col items-center gap-4">
        ...
        <H2>You now understand the Flywheel</H2>
      </div>
          ...
          {RELATED_MENTAL_MODELS.map((model) =>
            model.href ? (
              <Link
                key={model.name}
                href={model.href}
                className="group relative flex flex-col gap-2 rounded-2xl border border-border p-5 text-left transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:border-brand/25 hover:shadow-lg hover:shadow-brand/10"
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-serif text-lg font-medium group-hover:text-brand">{model.name}</span>
              ...
              <div
                key={model.name}
                className="group relative flex flex-col gap-2 rounded-2xl border border-dashed border-border p-5 text-left"
              >
                <div className="flex items-center justify-between gap-2">
