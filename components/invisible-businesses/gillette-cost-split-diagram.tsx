...
import { RefreshCw, ShoppingCart, Layers } from "lucide-react";
...
const bars = [
  {
    label: "Handle",
    detail: "Priced near cost",
    widthPct: 10,
    icon: ShoppingCart,
    tone: "muted" as const,
  },
  {
    label: "Refill Cartridges",
...
const loopNodes = [
  { label: "Buy The Handle", icon: ShoppingCart },
  { label: "Blade Dulls In Weeks", icon: Layers },
  { label: "Buy The Refill", icon: RefreshCw },
];
...
function GilletteCostSplitDiagram() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 text-center text-xs text-muted-foreground sm:text-sm">
        ...
        Churn reduction after Gillette launched its own subscription refill service.
      </div>
    </div>
  );
}

export { GilletteCostSplitDiagram };
