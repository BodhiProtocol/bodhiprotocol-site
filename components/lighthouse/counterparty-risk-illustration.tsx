...
const rows = [
  { label: "Exposure To", bilateral: "Specific counterparty", cleared: "Clearinghouse" },
  { label: "If They Default", bilateral: "You absorb the loss", cleared: "Loss mutualized" },
];

function CounterpartyRiskIllustration() {
  ...
  return (
    <div ref={ref} className="rounded-xl border border-border bg-muted p-4 font-mono text-[11px]">
      <div className="mb-3 grid grid-cols-[4.2rem_1fr_1fr] gap-2 text-[9px] font-bold tracking-wide">
        ...
        <span className="text-center text-rose-600 dark:text-rose-500">BILATERAL</span>
        <span className="text-center text-brand">CLEARED</span>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        {rows.map((row, i) => (
          ...
          >
            ...
            <span className="text-center text-[9px] font-bold text-brand">{row.cleared}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mt-3 flex items-center justify-between border-t border-dashed border-border pt-2.5">
        ...
        <span className="text-[9px] font-bold tracking-wide text-brand">DIFFERENT COUNTERPARTY</span>
      </div>
    </div>
  );
}

export { CounterpartyRiskIllustration };
