...
import { getPlaybookPtBrBySlug } from "@/lib/ba-playbooks-pt-br";
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          <div style={{ display: "flex", fontSize: 28, color: "#7c3aed" }}>BodhiProtocol</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ display: "flex", fontSize: 56, fontWeight: 600, lineHeight: 1.2 }}>
              {guide?.title ?? "BodhiProtocol"}
            </div>
            {guide ? (
              <div style={{ display: "flex", fontSize: 24, color: "#52525b" }}>
                {guide.hacks.length} práticas · {guide.readingTime}
              </div>
            ) : null}
          </div>
        </div>
