...
Think of a fruit seller who sells the same basket of mangoes at three different prices over one morning — a discount on the first dozen to draw a crowd, the regular price through the busy middle stretch, and a markdown on the last few before closing. Ask what the mangoes "really" sold for that day, and averaging the three prices equally would be wrong — most of the fruit moved at the middle price, so the true average has to weight each price by how many mangoes sold at it.

A trading day works the same way.
