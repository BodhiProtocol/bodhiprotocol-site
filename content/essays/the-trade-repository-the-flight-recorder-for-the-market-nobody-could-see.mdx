...
Nobody checks a flight recorder until something crashes. For years, nobody checked whether Barclays' swap reports were actually right, either — the pipe looked fine from the outside the entire time. India's regulator is now writing its own identifier fix into law before anything comparable gets the chance to happen at all. Both records exist for the identical reason: not to stop the next crisis, but to make sure that when someone finally has to ask what happened, an answer is sitting there waiting — and, increasingly, that the answer is actually correct.
