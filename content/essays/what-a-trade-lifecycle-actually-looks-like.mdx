...
hook: "A trade is not done when two people agree on price."
...
You've probably tracked an online order this way: Placed → Shipped → Out for Delivery → Delivered → Return window closes. A trade goes through the same shape of journey — just moving cash and securities instead of a parcel.
...
Order Initiation → Execution → Capture & Enrichment → Clearing → Settlement → Post-Settlement. Each stage hands the trade to a different system, a different desk, and often a different legal entity.
...
## Example 1: NSE settlement in India

On the NSE, this whole walk happens on a T+1 cycle: Order Initiation through Capture & Enrichment finish on trade day, and Clearing through Settlement complete by the next morning. NSE Clearing (NSCCL) sits at the Clearing stage as the central counterparty for every trade.

## Example 2: US equities internationally

The US ran a T+2 cycle for decades before shortening it to T+1 in 2024 — compressing the same six-stage walk by an entire day, for the same reason India's markets already ran it that fast: less time sitting between execution and settlement is less time for something to go wrong while it's exposed.

Two of the world's largest equity markets, arriving at the same one-day settlement cycle by different paths.
