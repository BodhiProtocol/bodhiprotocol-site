...
hook: "A market price is not discovered by magic. It is negotiated in public."
...
You've probably seen a wholesale vegetable market at dawn — several sellers calling out different asking prices for the same crate of tomatoes, several buyers each with their own top price in mind, everyone silently ranking the offers from best to worst. A trade happens the moment one buyer's top price meets one seller's bottom price.

A stock exchange runs the exact same thing — just organized into two neat, sorted lists instead of one noisy crowd.
...
## Example 1: NSE's order book in India

Open the market depth window for a stock like Reliance Industries on the NSE, and you're looking at exactly this structure, live: several levels of bids stacked below the current price, several levels of asks stacked above it, each showing the quantity waiting at that price. The exchange publishes this depth in real time specifically so that anyone — not just the exchange itself — can see how much genuine buying and selling interest sits behind the top price.

## Example 2: NASDAQ's order book internationally

The same structure runs a stock like Apple on NASDAQ, an ocean away and a different currency entirely. The bid/ask stacks, the spread, the depth behind the top price — none of that changes with geography. What changes is only the names on the tickets and the currency printed on them.

Two exchanges, two continents, the same two stacks of intent.
