...
This is exactly the failure mode the global LIBOR-to-SOFR transition exposed at scale. Legacy loans referencing LIBOR were converted to SOFR plus a fixed credit spread adjustment, chosen to approximate — not replicate — the old benchmark's historical relationship to the new one. A hedge booked against the old benchmark and an exposure now sitting on the new one can look perfectly matched in a system that only checks "is this rate risk," while carrying a small, permanent basis gap that never nets to zero. A risk system that flags a book as "flat" without separately validating benchmark, reset date and convention on both legs is reporting a hedge that's flat in name and not in cash.
