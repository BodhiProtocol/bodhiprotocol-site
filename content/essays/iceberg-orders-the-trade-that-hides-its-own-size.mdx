...
hook: "An order book shows what's for sale and how much. An iceberg order breaks that last promise on purpose."
...
Think of a shopkeeper who keeps only two units of a popular item on the counter, and the moment one sells, quietly brings out another from a stockroom nobody can see — so a customer glancing at the shelf only ever reads "two left," never the fifty actually sitting in the back.

An order book is supposed to work like an honest, fully-stocked shelf.
