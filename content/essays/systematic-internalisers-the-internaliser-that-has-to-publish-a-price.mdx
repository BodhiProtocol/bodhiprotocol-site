...
Picture that same office from before, with the two petty-cash drawers quietly moving euros between colleagues. Now imagine a regulator walks in and says: you're doing this so often now, and for so much money, that a private drawer isn't good enough anymore. You have to put a rate board on your desk — a real, tradeable number — and you have to honor it for any employee who walks up and asks, not just the two who happened to need euros that day.
...
A European investment firm's trade capture and transaction reporting system carries that harder case as a permanent structural feature. Under MiFIR's transaction reporting rules (RTS 22), every trade record needs a venue field, and that field isn't decorative: it can read as a regulated market, an MTF, an OTF, a Systematic Internaliser, or plain OTC, and each value routes to a different transparency and reporting regime. When an SI is one side of the trade, the SI itself typically carries the post-trade reporting obligation — meaning the trade capture system has to know, at the moment of execution, not just the price and counterparty but the *regulatory status* of the counterparty, because that status decides who is legally on the hook for reporting the trade at all. SIs are even assigned their own four-character Market Identifier Code, the same identifier family used for the London Stock Exchange (XLON) or the NSE (XNSE) — a trade against one is stamped with that firm-specific code, not a generic "OTC" flag.
...
- [EUR-Lex: Commission Delegated Regulation (EU) 2017/587 (RTS 1) — SI quoting thresholds and obligations](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32017R0587)
