...
Delete the document store mid-conversation, and the model doesn't gradually forget the facts it had been using — it simply stops having access to them, starting with the next query, nothing in between. That's the same hard, binary cutoff as [a context window running out](/essays/context-windows-arent-memory), and for a related reason: both get described with the word "memory," and in both cases the word is wrong for an identical cause — real memory fades, gradually and imperfectly. Neither of these does. A fact was either retrieved this query, or it never existed to the model at all.
