...
hook: "The market does not settle every promise. It compresses the mess."
...
You've probably split a group dinner bill this way without thinking about it: instead of everyone paying everyone back individually for the round they covered, the table just works out who owes what net, and only that one final amount changes hands.

Finance runs the same idea, at a scale where getting it wrong can break a market.
...
## Example 1: CCIL in India

Every day, banks across India trade government securities and foreign exchange with each other by the thousands.

The Clearing Corporation of India Ltd (CCIL) sits in the middle of that activity as a central counterparty, and one of its core jobs is multilateral netting: instead of every bank settling every trade with every other bank individually, CCIL works out each member's net position across the whole market and settles only that final number.

A bank that bought and sold government securities all day doesn't settle each trade one by one. It settles its one net obligation with CCIL.

## Example 2: CLS Bank internationally

Move the same problem to the global foreign exchange market, where banks in different time zones are trading currencies against each other around the clock.

CLS Bank was built specifically to solve the netting-and-settlement problem in FX — a member bank's trades in a given currency, across every counterparty it dealt with that day, are netted down to one payment per currency, instead of settling every individual trade separately.

That compression is also what removed a specific old danger from the FX market: a bank paying out the currency it owed before it could confirm it was actually receiving the currency owed to it in return.
