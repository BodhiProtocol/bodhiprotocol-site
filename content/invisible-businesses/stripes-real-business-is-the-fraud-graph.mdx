...
insights:
  ...
  - icon: "📉"
    text: "Radar reduces fraud by an average of 38% — a reduction no merchant's own historical data, however large, could produce working alone."
  - icon: "💰"
...
The result shows up as a number that's hard to argue with: Radar reduces fraud by an average of 38%. No merchant, however large, generates enough of its own transaction volume to train a model that good in isolation. It only works because the network is shared.
