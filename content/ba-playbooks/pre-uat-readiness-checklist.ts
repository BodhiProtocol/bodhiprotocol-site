...
Environment: a bad environment can make a good feature look broken. Check the correct build is deployed, the environment is accessible, the database is available, required services are running, there's no known blocking outage, test data was refreshed correctly, URLs and endpoints are correct, and no uncoordinated changes are happening during critical testing. It doesn't need to be perfect — it needs to be stable enough that failure means something.

Access and roles: never assume testers had access last time. Confirm users are created, they can log in, roles and permissions are correct, they can reach dependent systems, MFA and VPN work, and segregation-of-duties rules are respected. Eight business users discovering an access problem during the UAT call isn't testing — it's troubleshooting.

Integrations: your feature may work perfectly, but UAT usually tests a journey — portal to verification API to core system to notification service. If the notification service isn't connected, the journey isn't ready. Ask what other system must work for this scenario to finish, and check critical upstream and downstream dependencies before testers arrive.

Configuration: feature flags, thresholds, business rules, product configuration, reference data, routing, currencies, dates and user entitlements can quietly invalidate otherwise perfect testing. If the requirement says verification above ₹2 lakh but UAT is configured for ₹5 lakh, the code may be perfect and the test will still tell the wrong story.

Stakeholders: technology can be ready while UAT still fails operationally. Before starting, everyone should know what's being tested, who's testing what, where results are recorded, how defects are raised, who triages them, what counts as a blocker, and who decides acceptance — plus the support path across engineering, QA, BA, environment, data and integration teams.
