...
- **Every essay, not just one section**: pair one Indian example with one international example throughout the piece — every desk/concept/mechanism gets both, not just the "Why markets needed this" section (e.g. an MCX gold futures hedge next to an ICE coffee futures hedge; RBI's insurable-interest rule for CDS next to the pre-2008 naked-CDS US market). International example first, then Indian, is the established order in every case — including when a mechanism is native to one jurisdiction and has no real Indian counterpart (e.g. a MiFID II-specific category like a Systematic Internaliser): lead with the market where the thing exists, then bring in the Indian side (why it doesn't exist there, or the closest structural analogue). This is load-bearing for the site's positioning — don't drop to a single-market example for a new essay.
