...
- **Essay readership span**: every essay must work for a total beginner (someone who's never heard the term) through to a deep professional (analyst, QA/tester, researcher) in the same piece — not a beginner version and a separate expert version. The mechanism: open every dense concept with a plain "Think of X..." everyday analogy (a fixed deposit, a pizza, two friends lending money, two bosses) before naming the technical term, then let the technical depth (mechanics, settlement, regulation) sit right after it rather than being cut for simplicity. If a section only works for one end of that range, it needs an analogy pass, not a rewrite into two versions.
- **"Ten by ten" length target**: essays should read in roughly 10 minutes across roughly 10 sections (H2 headings, cold open not counted) — compact enough to stay skimmable, long enough to actually cover the topic with both examples and both reading levels. Not a hard rule to pad or trim to exactly 10; a guide for scoping how much a single essay should try to cover.
